    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (string mail in mails)
            {
                ...
                string sameMail = mail.Substring(mail.IndexOf("@")+1);
                string mail2 = mail.Remove(mail.LastIndexOf("@")+1);

                for (int i = 0; i < number.Length; i++)
                {
                    if (mail2.Contains(number[i]))
                    {
                        mail2 = mail2.Replace(number[i], "");
                    }
                }

                mail2 = mail2 + sameMail;
                if (mail2.Contains(character))
                {
                    mail2 = mail2.Replace(character, ".");
                }
                if (mail2.IndexOf(".") != mail2.LastIndexOf("."))
                {
                    Array.Resize(ref sortedArray , sortedArray.Length+1);    
                    sortedArray[sortedArray.Length-1] = mail2;
                }
            }


            foreach(string sortedMail in sortedArray)
            {
                string ad = sortedMail.Remove(sortedMail.IndexOf("."));
                string soyad = sortedMail.Substring(sortedMail.IndexOf(".")+1, sortedMail.IndexOf("@") - sortedMail.IndexOf(".")-1);
